...
def epochVal(model, dataLoader, optimizer, scheduler, loss):
    ...
    for i, (input, target) in enumerate (dataLoader):
        ...
        target = target.view(-1, 13).contiguous().cuda(async=True)
    ...
    auc = computeAUROC(outGT, outPRED, 13)
...
def train_one_model(model_name):
    ...
    snapshot_path = path_data['snapshot_path'] + model_name + '_' + str(Image_size) + '_chexpert'
    ...
    for num_fold in range(5):
        ...
        train_loader, val_loader = generate_dataset_loader_chexpert(df_all, c_train, train_transform, train_batch_size, c_val, val_transform, val_batch_size, workers)

        model = DenseNet121_change_avg(13, True)
        model = torch.nn.DataParallel(model).cuda()

        optimizer = torch.optim.Adamax(model.parameters(), lr=0.0005, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
        ...
        for epochID in range (0, trMaxEpoch):
            ...
            if (epochID+1)%5 == 0 or epochID > 24 or epochID == 0:
...
if __name__ == '__main__':
    ...
    import json
    f1 = open('../configs/path_configs_chexpert.json', encoding='utf-8')
