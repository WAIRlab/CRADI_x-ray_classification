...
def epochVal(model, dataLoader, optimizer, scheduler, loss):
    ...
    for i, (input, target) in enumerate (dataLoader):
        ...
        target = target.view(-1, 10).contiguous().cuda(async=True)
    ...
    auc = computeAUROC(outGT, outPRED, 10)
...
def train_one_model(model_name):
    ...
    snapshot_path = path_data['snapshot_path'] + model_name + '_' + str(Image_size) + '_10_local_val'
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)
    header = ['Epoch', 'Learning rate', 'Train Loss', 'Val Loss', 'Time', 'F1 score', 'AUC'] + list(range(10)) + list(range(10)) + list(range(10)) + list(range(10)) + list(range(10))
    ...
    for num_fold in range(5):
        ...
        train_loader, val_loader = generate_dataset_loader_10(df_all, c_train, train_transform, train_batch_size, c_val, val_transform, val_batch_size, workers)
...
if __name__ == '__main__':
    ...
    import json
    f1 = open('../configs/path_configs_10.json', encoding='utf-8')
